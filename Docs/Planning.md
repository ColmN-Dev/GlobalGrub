...
The visual theme for GlobalGrub is a forest theme, inspired by earthy, natural tones. The colour scheme is built around olive green and deep brown. The navbar will have a leaf pattern background and the search bar area will use a similar treatment. Recipe cards in the hardcoded carousel will have a wood bark texture background to tie into the overall theme. The main page background will be a daytime forest image.

The app will support both light mode and dark mode. In dark mode the background will switch to a nighttime forest image and the navbar and UI colours will adjust accordingly. The dark mode toggle will be accessible from the navbar. This functionality will be adapted from a previous JavaScript project I completed, as will the carousel logic, which will save development time and build on code I already know works.

The hero banner on the home page uses a detailed globe with a plate, knife and fork icon that represents the global food theme. A simpler, cleaner version of this logo is used as the favicon and brand icon, as the detailed version does not scale well at small sizes. Having two versions of the logo gives flexibility across different contexts in the app.
...
The database will have two tables. The first is a `Users` table which will store each user's `id`, `username`, hashed `password`, and their chosen `avatar`. The second is a `Favourites` table which will store saved recipes, with each row containing a `meal_id`, `meal_name`, `meal_image`, and a `user_id` that links back to the `Users` table as a foreign key. This creates a relationship between the two tables so that each saved recipe is tied to a specific user account. This makes GlobalGrub a relational database application.
...
Form validation will be included on the signup and login pages. This will check that fields are not empty, that the username is not already taken, and that the password meets a minimum strength requirement. Error messages will be shown to the user if validation fails.
...
The Home page will be the first thing built after `base.html` is set up, as it establishes the overall look and feel of the app. It will have a navbar at the top that stays fixed as the user scrolls down. On mobile the navbar will collapse into a hamburger icon that opens a slide out drawer menu from the side. Below the navbar there will be a hero section with a bold headline and short tagline explaining what GlobalGrub is, with the detailed globe and fork logo as the hero banner image. Underneath that there will be a marquee strip of food images that scrolls continuously using a CSS animation. Below the marquee there will be three large clickable cards representing the three regions — Asian, European, and American — which all link through to the Recipes page. At the bottom of the page will be the footer with links to the main pages of the site and a copyright line. I will include cookies in my website to allow for Flask to handle the session.

## The Recipes Page

The Recipes page is the most complex part of the app. At the top there will be a search bar that connects to the `TheMealDB` API, which is a free API that returns meal data including names, images, ingredients and instructions. When a user types in a search term and submits, Flask will make a request to the API on the backend and pass the results back to the template to display as recipe cards.

Below the search bar there will be a curated carousel section which will be hardcoded by me with dishes I have researched myself. The carousel will be organised by three world regions: Asian, European, and American. Each region will have eight hardcoded dishes. By default when the page loads the All Regions view is active and the carousel shows 12 dishes in total — one meat dish, one fish dish, one vegetarian dish and one dessert from each of the three regions, giving four dishes per region and twelve overall. When a user clicks on a specific region the JavaScript `updateCarousel` function will filter the display to show all eight dishes from that region only. There will also be an All Regions button that resets back to the default mixed view. Each dish card will have a wood bark texture background, an image, a title, a category label and a link to a reputable external website like BBC Food or AllRecipes where users can get the full recipe. The carousel logic will be adapted from a previous project I completed.
...
The Profile page will be a dashboard for logged in users. At the top there will be a profile card showing the user's current avatar displayed as a circle and their username. Below that there will be a stat showing how many recipes they have saved in their favourites. The page will also have a form to edit their username, a form to change their password, and a button to open the avatar picker modal. There will also be a logout button that clears the session and redirects them to the home page. Any changes to the username or password will be handled as `POST` requests in Flask and will update the relevant fields in the `SQLite` database.

## The Avatar System

I want the avatar picker to work similar to how PlayStation handles profile avatars, where a modal pops up and the user can choose from a set of preset icons. I am planning to have around eight to ten food themed icons to choose from, things like a pizza slice, a taco, a rotisserie chicken, a burger and so on. The default will be a plain silhouette icon. All avatar icons will be displayed as circles to match modern profile image conventions. When a user picks a new avatar the choice will be saved to their record in the `SQLite` database and the UI will update instantly without needing a full page refresh. The selected avatar will appear in the navbar and on the profile page.

## The About Page

The About page will be built last as it is the simplest page in the app. It will have a short explanation of what GlobalGrub is, how it works, and credit to the `TheMealDB` API. It may also include a small section about the developer with a circular profile image and a brief blurb, similar to an author bio on a book.

## CSS Styling

All styling for GlobalGrub will be written by me in `style.css` located in the `static/css` folder. I will link this file to every HTML page through `base.html`. I plan to use a range of CSS selectors, classes and IDs to target specific elements across the pages. I will make use of `flexbox` and `CSS grid` for layout, and add transitions and hover effects to make the interface feel interactive and polished. The forest theme will be applied through background images, texture overlays and the olive green and deep brown colour palette. The fixed navbar, the marquee animation, the slide out mobile menu, the dark mode toggle and the modal will all be styled in this file.

## JavaScript

Most of the interactive behaviour in the app will be handled by `script.js` located in the `static/js` folder. This file will be linked to every HTML page through `base.html` just before the closing `body` tag. The JavaScript will cover the carousel `updateCarousel` function for switching between regions, the dark mode toggle, the slide out mobile hamburger menu, the avatar modal for opening, selecting and updating  and dynamic UI updates like toggling the save button state on recipe cards. Where possible, code will be adapted from my previous JavaScript project to save time and build on working solutions.
...
![Recipes Page Wireframe](wireframes/recipes.png)
